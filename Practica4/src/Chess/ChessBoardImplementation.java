...
public class ChessBoardImplementation implements ChessBoard {
	...
	public ChessBoardImplementation() {
        ...
        /*
        This method checks if there's any pawn on the borders and transforms it into a queen
        */
	public void pawnTransformation (){
            for (int i = 0; i < 8; i++) {
                if(pieces[getPieceIndex(i, 0)].getType() == ChessPiece.Type.PAWN) {
                    ChessPiece.Color thatColor = pieces[getPieceIndex(i, 0)].getColor();
                pieces[getPieceIndex(i, 0)] = new ChessPieceImplementation(thatColor,ChessPiece.Type.QUEEN);
            }
           }  
            for (int i = 0; i < 8; i++) {
                if(pieces[getPieceIndex(i, 0)].getType() == ChessPiece.Type.PAWN) {
                    ChessPiece.Color thatColor = pieces[getPieceIndex(i, 8)].getColor();
                pieces[getPieceIndex(i, 8)] = new ChessPieceImplementation(thatColor,ChessPiece.Type.QUEEN);
            }
           }
        }
	@Override
